...
function setupImageTrails() {
  ...
  cards.forEach((card) => {
    const image = card.querySelector('.waterfall-image img, :scope > img:not(.trail-clone)');
    if (!image) return;
    ...
    if (!context || !brushContext) return;
    ...
    const droplets = [];
    const lastPointer = { x: 0, y: 0, time: 0, active: false };
    let frame = 0;
    let previousTime = 0;
    let width = 0;
    let height = 0;
    ...
    const imageRect = () => {
      const naturalWidth = image.naturalWidth || Number(image.width) || width;
      const naturalHeight = image.naturalHeight || Number(image.height) || height;
      const contain = Boolean(image.closest('.waterfall-image'));
      const scale = contain
    ...
    const drawDroplet = (droplet) => {
      const radius = droplet.radius * (0.82 + (1 - droplet.life) * 0.24);
      const diameter = Math.ceil(radius * 2);
      const brushRatio = pixelRatio;
      const physicalSize = Math.max(2, Math.ceil(diameter * brushRatio));
      if (brush.width !== physicalSize || brush.height !== physicalSize) {
        brush.width = physicalSize;
        brush.height = physicalSize;
      }
      brushContext.setTransform(brushRatio, 0, 0, brushRatio, 0, 0);
      brushContext.clearRect(0, 0, diameter, diameter);

      const source = imageRect();
      const wobble = Math.sin(droplet.phase + (1 - droplet.life) * 5.4) * radius * 0.08;
      const offsetX = droplet.vx * droplet.life * 1.7 + wobble;
      const offsetY = droplet.vy * droplet.life * 1.7 - wobble * 0.55;
      brushContext.filter = 'saturate(1.18) contrast(1.06)';
      brushContext.drawImage(
        image,
        source.x - droplet.x + radius + offsetX,
        source.y - droplet.y + radius + offsetY,
        source.width,
        source.height,
      );
      brushContext.filter = 'none';
      brushContext.globalCompositeOperation = 'destination-in';
      const mask = brushContext.createRadialGradient(radius, radius, radius * 0.05, radius, radius, radius);
      mask.addColorStop(0, 'rgba(255,255,255,0.94)');
      mask.addColorStop(0.5, 'rgba(255,255,255,0.7)');
      mask.addColorStop(0.82, 'rgba(255,255,255,0.24)');
      mask.addColorStop(1, 'rgba(255,255,255,0)');
      brushContext.fillStyle = mask;
      brushContext.fillRect(0, 0, diameter, diameter);
      brushContext.globalCompositeOperation = 'source-over';

      context.globalAlpha = Math.min(0.74, droplet.life * 0.82);
      context.drawImage(brush, droplet.x - radius, droplet.y - radius, diameter, diameter);
    };
    ...
    const render = (time) => {
      ...
      for (let index = droplets.length - 1; index >= 0; index -= 1) {
        ...
        droplet.life -= delta / droplet.duration;
        if (droplet.life <= 0) {
      ...
      if (droplets.length) frame = requestAnimationFrame(render);
      else {
        frame = 0;
        previousTime = 0;
      }
    ...
    const addTrail = (x, y, vx, vy) => {
      const distance = lastPointer.active ? Math.hypot(x - lastPointer.x, y - lastPointer.y) : 0;
      const steps = Math.max(1, Math.min(6, Math.ceil(distance / 14)));
      for (let step = 1; step <= steps; step += 1) {
        const progress = step / steps;
        droplets.push({
          x: lastPointer.active ? lastPointer.x + (x - lastPointer.x) * progress : x,
          y: lastPointer.active ? lastPointer.y + (y - lastPointer.y) * progress : y,
          vx,
          vy,
          radius: Math.max(34, Math.min(92, width * (0.105 + Math.random() * 0.035))),
          duration: 620 + Math.random() * 260,
          phase: Math.random() * Math.PI * 2,
          life: 1,
        });
      }
      if (droplets.length > 28) droplets.splice(0, droplets.length - 28);
    ...
    card.addEventListener('pointermove', (event) => {
      ...
      const x = clamp(localX / rect.width) * 2 - 1;
      const y = clamp(localY / rect.height) * 2 - 1;
      const depth = Number(card.dataset.depth || 0.7);
      const elapsed = Math.max(12, event.timeStamp - (lastPointer.time || event.timeStamp - 16));
      const velocityX = lastPointer.active ? clamp((localX - lastPointer.x) / elapsed, -1.8, 1.8) * 18 : 0;
      const velocityY = lastPointer.active ? clamp((localY - lastPointer.y) / elapsed, -1.8, 1.8) * 18 : 0;
      card.style.setProperty('--media-x', `${(x * 8 * depth).toFixed(2)}px`);
      card.style.setProperty('--media-y', `${(y * 5 * depth).toFixed(2)}px`);
      addTrail(localX, localY, velocityX, velocityY);
      lastPointer.x = localX;
      lastPointer.y = localY;
      lastPointer.time = event.timeStamp;
